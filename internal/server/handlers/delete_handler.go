...
// 示例请求: DELETE /api/link?link=https://a.com&link=https://b.com
func DeleteHandler(c echo.Context) error {

	// 从查询参数中获取所有 "link=" 参数值
	links := c.QueryParams()["link"] // 获取同名多个 query 值
	if len(links) == 0 {
		return c.JSON(http.StatusBadRequest, types.DeleteResponse{
			Code:    -1,
			Message: "Missing required query parameter 'link'",
		})
	}
	...
	for _, rawLink := range links {
		...
		if err != nil || (u.Scheme != "http" && u.Scheme != "https") {
			return c.JSON(http.StatusBadRequest, types.DeleteResponse{
				Code:    -1,
				Message: "Invalid URL format: " + rawLink,
			})
		}
	}
