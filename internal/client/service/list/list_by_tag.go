...
func ListByTag(addr string, accessToken string, tag string, page int, pageSize int) (response.Response, error) {
	// 构建请求路径和查询参数
	pathWithQuery := fmt.Sprintf("/api/v1/tags/%s?page=%d&page_size=%d", tag, page, pageSize)
